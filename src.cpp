...
{
	for (int i = 0; i < H / 2; i++) { // y좌표. 맨 윗줄과 아랫줄 서로 바꿈. 그래서 1/2만 해도 됨. 
		for (int j = 0; j < W; j++) { // x좌표
			swap(&Img[i*W + j], &Img[(H-1-i)*W + j]); // 맨 위 첫번째랑 맨 아래 첫번째 셀 바꾸기.. 
		}
	}
}
...
{
	// Translation
	Ty *= -1; // BMP 영상은 위아래 뒤집혀있으므로 y 방향은 부호를 생각해야주어야함. 
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if ((i + Ty < H && i + Ty >= 0) && (j + Tx < W && j + Tx >= 0)) // 영역에 대한 클리핑 처리
				 // x와 y 모두 만족해야하므로 && 사용. 
...
void Scaling(BYTE* Image, BYTE* Output, int W, int H, double SF_X, double SF_Y)
{
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			tmpX = (int)(j / SF_X);
			tmpY = (int)(i / SF_Y); // hole 문제 발생 가능 있음. 나한테 와야하는 애는 누구인지 역으로 계산.
			// 역방향 사상. 스케일링 factor로 나눠줌. 
			// 역행렬 계산. 
			if (tmpY < H && tmpX < W)
...
void Rotation(BYTE* Image, BYTE* Output, int W, int H, int Angle)
{
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			tmpY = (int)(-sin(Radian) * j + cos(Radian) * i); // 역방향 사상. 
			//tmpX = (int)(cos(Radian) * j - sin(Radian) * i);
...
int main()
{
	...
	//Translation(Image, Output, W, H, 100, 40); // y 방향 이동시 부호를 바꿔서 생각해야함. 
